...
async function github(url, options = {}) {
  ...
  if (!token) throw Object.assign(new Error("GITHUB_TOKEN não está configurado na Vercel."), { status: 500 });
  ...
  if (!res.ok) {
    throw Object.assign(
      new Error(body?.message || text || `GitHub HTTP ${res.status}`),
      { status: res.status }
    );
  }
...
async function readRemote() {
  ...
  try {
    // Read the file as raw JSON. This avoids Base64 decoding issues.
    ...
    try {
      ...
      throw Object.assign(
        new Error("O backup existente no GitHub não contém JSON válido."),
        { status: 409 }
      );
    }
    ...
    // First use: the backup file may not exist yet.
...
async function saveRemote(stores, version) {
  ...
  return {
    ...
    path: result.body?.content?.path || FILE_PATH
  };
}
...
export default async function handler(request) {
  try {
    if (request.method === "GET") {
      ...
      const stores =
        payload?.stores && typeof payload.stores === "object"
          ? payload.stores
          : payload;
    ...
    if (request.method === "POST") {
      ...
      try {
        ...
        return json({ error: "Pedido POST não contém JSON válido." }, 400);
      }
      ...
      return json(await saveRemote(body.stores ?? {}, body.version ?? 1));
    }

    return json({ error: "Método não suportado." }, 405);
  } catch (error) {
    console.error("VIKING Vercel sync error:", error);
    const status =
    ...
    return json({
      error: error?.message || "Erro de sincronização."
    }, status);
  }
}
